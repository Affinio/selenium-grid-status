...
function parseConfig(items) {
    ...
    items.forEach(function(item) {
        ...
        item.children.forEach(function(config) {
            // Really only want the first item from the ':', so join the rest back up
            // so the value can have a ':' in it
            var data = config.children[0].data.split(/:/);
            nodeConfig[data.shift()] = data.join(":");
        });
...
exports.available = function(options, callback) {
    ...
    if(options.protocol === 'https'){
      https.get(options.fullUrl, successCallback).on('error', callback);
    } else {
      http.get({
        host: options.host,
        path: '/grid/console',
        port: options.port || 4444
      }, successCallback).on('error', callback);
    }
};
